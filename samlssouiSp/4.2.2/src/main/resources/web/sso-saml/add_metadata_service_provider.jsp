...
<%
        ...
        SAMLSSOServiceProviderDTO retrievedObject = null;
        ...
        String cookie1;
        SAMLSSOConfigServiceClient clientmetadata;

        backendServerURL = CarbonUIUtil.getServerURL(config.getServletContext(), session);
        configContext1 = (ConfigurationContext) config.getServletContext().getAttribute(CarbonConstants.CONFIGURATION_CONTEXT);
        cookie1 = (String) session.getAttribute(ServerConstants.ADMIN_SERVICE_COOKIE);
        String BUNDLE1 = "org.wso2.carbon.identity.sso.saml.ui.i18n.Resources";
        ResourceBundle resourceBundle1 = ResourceBundle.getBundle(BUNDLE1, request.getLocale());
        clientmetadata = new SAMLSSOConfigServiceClient(cookie1, backendServerURL, configContext1);

    System.out.println("#####"+spAction);
    ...
    if (spAction != null && "addServiceProviderFromFile".equals(spAction)) {

            System.out.println("addmetadatafile jsp entered");

            fileContent = SAMLSSOUIUtil.getFileContent(request, request.getParameter("metaFile"));
            System.out.println(fileContent);
            session.setAttribute("fileContent",fileContent);

            retrievedObject = clientmetadata.addMetadataServiceProviderSAMLObjectOnly(fileContent);
            System.out.println(retrievedObject);

            provider = retrievedObject;
            System.out.println("###################1"+provider.getIssuer());
            System.out.println("###################2"+provider.getAssertionConsumerUrl());
            System.out.println("###################3"+provider.getNameIDFormat());
            System.out.println("###################4"+provider.getUseFullyQualifiedUsernameS());
...
<tr>
    ...
    <td><input type="text" id="issuer" name="issuer" maxlength="100"
               class="text-box-big"
               value="<%=provider.getIssuer()%>" <%=isEditSP ? "disabled=\"disabled\"" : ""%>/>
...
<tr>
    ...
    <td>
        <input type="text" id="assrtConsumerURL"
               ...
               value="<%=provider.getAssertionConsumerUrl()%>"/>
    </td>
</tr>
