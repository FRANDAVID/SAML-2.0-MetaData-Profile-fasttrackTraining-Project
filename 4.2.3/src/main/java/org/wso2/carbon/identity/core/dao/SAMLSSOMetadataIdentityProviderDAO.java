...
public class SAMLSSOMetadataIdentityProviderDAO extends AbstractDAO {
     ...
     */
    public boolean addMetadataIdentityProvider(String fileContent, String issuer) throws IdentityException {
        String path = null;
        Resource resource;
        System.out.println(issuer);
        System.out.println(fileContent);
        System.out.println("add metadata registry hit");

        if (fileContent != null && issuer != null) {

            path = IdentityRegistryResources.SAML_SSO_METADATA_IDENTITY_PROVIDERS_INBOUND + encodePath(issuer);
        }
        ...
        try {
            if (registry.resourceExists(path)) {
                if (log.isDebugEnabled()) {
                    ...
                    System.out.println("the name was already there");
                }
                return false;
            }
